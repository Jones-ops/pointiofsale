...
const pricelistModel = {
  ...
  async findById(id) {
    const pl = await db.one('SELECT * FROM pricelists WHERE id = ?', [id]);
    if (!pl) return null;
  ...
  async update(id, data) {
    ...
    await db.run(`UPDATE pricelists SET ${sets} WHERE id = ?`, [...Object.values(fields), id]);
    return pricelistModel.findById(id);
  },

  async remove(id) {
    await db.run('DELETE FROM pricelist_items WHERE pricelist_id = ?', [id]);
    await db.run('DELETE FROM pricelists WHERE id = ?', [id]);
  },
  ...
  async updateItem(id, data) {
    ...
    const sets = Object.keys(fields).map(k => `${k} = ?`).join(', ');
    await db.run(`UPDATE pricelist_items SET ${sets} WHERE id = ?`, [...Object.values(fields), id]);
    return db.one('SELECT * FROM pricelist_items WHERE id = ?', [id]);
  },

  async removeItem(id) {
    await db.run('DELETE FROM pricelist_items WHERE id = ?', [id]);
  },

  async calculatePrice(productId, customerId, quantity = 1) {
    if (!customerId) return null;
    const customer = await db.one('SELECT pricelist_id FROM customers WHERE id = ?', [customerId]);
    if (!customer || !customer.pricelist_id) return null;

    const pricelist = await db.one('SELECT * FROM pricelists WHERE id = ? AND is_active = 1', [customer.pricelist_id]);
    if (!pricelist) return null;

    const product = await db.one('SELECT * FROM products WHERE id = ?', [productId]);
