...
const sessionModel = {
  async open({ user_id, opening_cash, notes }) {
  ...
  async findById(id) {
    ...
    session.cash_moves = await sessionModel.getCashMoves(id);
    session.sales = (await db.all(
  ...
  async close(id, { closing_cash, notes }) {
    const session = await sessionModel.findById(id);
    if (!session) throw new Error('Session not found');
    if (session.status === 'closed') throw new Error('Session already closed');

    const cashMovesIn = await db.one("SELECT COALESCE(SUM(amount),0) as total FROM cash_moves WHERE session_id = ? AND type = 'in'", [id]);
    const cashMovesOut = await db.one("SELECT COALESCE(SUM(amount),0) as total FROM cash_moves WHERE session_id = ? AND type = 'out'", [id]);
  ...
  async addCashMove({ session_id, type, amount, reason }) {
    const session = await db.one("SELECT * FROM pos_sessions WHERE id = ?", [session_id]);
    if (!session) throw new Error('Session not found');
