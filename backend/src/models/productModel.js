...
const productModel = {
  findAll({ search, category_id, active, page = 1, limit = 50, customer_id } = {}) {
    ...
    if (customer_id) {
      for (const p of products) {
        const calc = pricelistModel.calculatePrice(p.id, customer_id);
  ...
  count({ search, category_id, active } = {}) {
  ...
  findById(id, { customer_id } = {}) {
    const product = db.one(
      'SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.id = ?',
      [id]
    );
    if (product && customer_id) {
      const calc = pricelistModel.calculatePrice(product.id, customer_id);
  ...
  update(id, data) {
    ...
    db.run(`UPDATE products SET ${sets}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, vals);
  ...
  adjustStock(id, quantity, type, reference, notes, staff_id) {
