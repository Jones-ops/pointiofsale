...
export default function Products() {
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      if (!confirm(`Import ${items.length} products? This will be applied to the database.`)) return;
      setImporting(true);
      const { data } = await api.post('/import/products', { items, overwrite: true });
  ...
  const columns = [
    ...
    { key: 'selling_price', label: 'Price', render: (v) => new Intl.NumberFormat().format(v) },
  ...
  return (
    <div>
      ...
      <ConfirmDialog open={!!confirmDelete} onClose={() => setConfirmDelete(null)} onConfirm={handleDelete}
        title="Delete Product" message={`Delete "${confirmDelete?.name}"? This cannot be undone.`} />
    </div>
  );
}
